...
func TestDataModelDataSource(t *testing.T) {
	...
	mock.Mux.HandleFunc("/v2/dataModels/data-model-1", func(response http.ResponseWriter, request *http.Request) {
		mock.AssertBearer(t, request)
		write(response, dataModel)
	})
	config := documentProviderConfig(mock) + `
data "sigma_data_model" "one" {
  id = "data-model-1"
}
`
	resource.UnitTest(t, documentTestCase([]resource.TestStep{{
		...
		Check: resource.ComposeAggregateTestCheckFunc(
			resource.TestCheckResourceAttr("data.sigma_data_model.one", "name", "Sales Model"),
		),
	}}))
}
